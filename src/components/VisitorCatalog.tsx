      ...
      {activeTab === 'courses' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8" id="courses-grid">
          {filteredCourses.map(course => {
            ...
            const isCampaignActive = course.freeModules && course.freeModules.length > 0;

            return (
              <div 
                key={course.id}
                className="bg-white rounded-3xl overflow-hidden border border-brand-wood/5 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between"
              >
                <div>
